...
# Remove all clarification_gate_node definitions
content = re.sub(r'async def clarification_gate_node\(state: AgentState\) -> dict\[str, Any\]:.*?(?=async def blueprint_phase_node\(state: AgentState\) -> dict\[str, Any\]:)', '', content, flags=re.DOTALL)
...
with open('backend/src/runtime/nodes.py', 'w', encoding='utf-8') as f:
    f.write(content)
print("Cleaned and updated nodes.py")
