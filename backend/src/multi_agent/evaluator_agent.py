"""
ASEP — Evaluator Agent
"""

import asyncio
import logging

from src.multi_agent.contracts import AgentRole, Message, MessageType
from src.multi_agent.coordination import CoordinationContext
from src.multi_agent.handoff import HandoffManager
from src.multi_agent.messaging import MessageBus
from src.evaluation.evaluator import Evaluator
from src.evaluation.datasets import EvaluationCase

logger = logging.getLogger(__name__)


class EvaluatorAgent:
    """Wraps the Phase 1.9 Evaluator Engine into a messaging-driven autonomous agent."""

    def __init__(self, bus: MessageBus, evaluator: Evaluator) -> None:
        self.role = AgentRole.EVALUATOR
        self._bus = bus
        self._evaluator = evaluator
        self._task: asyncio.Task | None = None

    def start(self) -> None:
        self._task = asyncio.create_task(self._run())

    async def _run(self) -> None:
        async for msg in self._bus.subscribe(self.role):
            if msg.message_type == MessageType.HANDOFF:
                await self._handle_handoff(msg)

    async def _handle_handoff(self, msg: Message) -> None:
        logger.info(f"[{msg.session_id}] EVALUATOR starting evaluation.")
        ctx = CoordinationContext(
            session_id=msg.session_id,
            run_id=msg.run_id,
            thread_id=msg.thread_id,
            trace_id=msg.trace_id,
            goal=msg.payload.get("goal", "")
        )

        try:
            # Reconstruct the case from payload or generate a dummy one if it was ad-hoc
            case = EvaluationCase(id=msg.session_id, goal=ctx.goal)
            
            result = await self._evaluator.evaluate_case(case)
            
            handoff_msg = HandoffManager.create_handoff(
                context=ctx,
                from_role=self.role,
                to_role=AgentRole.SUPERVISOR,
                payload={"evaluation_result": result.model_dump(mode="json"), "status": "completed"}
            )
            await self._bus.publish(handoff_msg)
            
        except Exception as exc:
            logger.error(f"[{msg.session_id}] EVALUATOR failed: {exc}")
            error_msg = HandoffManager.create_handoff(
                context=ctx,
                from_role=self.role,
                to_role=AgentRole.SUPERVISOR,
                payload={"error": str(exc), "status": "failed"}
            )
            await self._bus.publish(error_msg)
