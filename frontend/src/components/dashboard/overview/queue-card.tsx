...
export function QueueCard({ pendingCount }: { pendingCount: number }) {
  ...
  return (
    <Card className={`hover:border-primary/20 transition-colors h-full flex flex-col ${hasPending ? 'border-orange-500/30 bg-orange-500/5' : ''}`}>
      <CardHeader className="flex flex-row items-center justify-between pb-2">
        <CardTitle className="text-sm font-medium text-muted-foreground">Approval Queue</CardTitle>
        <ShieldAlert className={`w-4 h-4 ${hasPending ? 'text-orange-500' : 'text-muted-foreground'}`} />
      </CardHeader>
