...
export function AuditFeedCard() {
  const mockActivities = [
    { id: 1, action: "READ_SECRET api_stripe_key", actor: "agent_billing_99", time: "5 min ago", status: "success" },
    { id: 2, action: "TERMINATE_INSTANCE i-0abcd...", actor: "agent_infra_05", time: "1 hour ago", status: "blocked" },
    { id: 3, action: "UPDATE_MEMORY mem_e_01", actor: "agent_core_01", time: "2 hours ago", status: "success" },
  ]
  ...
  return (
    <Card className="hover:border-primary/20 transition-colors col-span-full md:col-span-1 lg:col-span-1 flex flex-col h-full">
      <CardHeader className="flex flex-row items-center justify-between pb-2">
        <CardTitle className="text-sm font-medium text-muted-foreground">Recent Audit Activity</CardTitle>
            ...
            <div key={activity.id} className="flex items-start gap-3">
              ...
              <div className="space-y-1">
                ...
                <div className="flex items-center gap-2 text-[10px] text-muted-foreground">
                  ...
                  <span>•</span>
                  <span className={activity.status === "blocked" ? "text-orange-500" : "text-emerald-500"}>
                    {activity.status}
                  </span>
                </div>
              </div>
